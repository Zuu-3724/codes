...
JWT_ALGORITHM = 'HS256'  # os.getenv('JWT_ALGORITHM', 'HS256')
...
async def login_json(credentials: Dict[str, Any]):
    ...
    try:
        ...
        logger.info(
            f"Received credentials: username={username}, password=****")
    ...
    try:
        ...
        try:
            ...
            if not stored_password.startswith('$2b$'):
                logger.error(
                    f"Invalid password hash format for user: {username}")
                raise HTTPException(
                    status_code=401,
                    detail={"Status": False,
...
async def get_roles():
    ...
    try:
        # Match with CHK_Role constraint
