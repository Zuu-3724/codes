...
async def get_departments(request: Request):
    ...
    try:
        ...
        query = """
            ...
            LEFT JOIN [HUMAN].[dbo].[Employees] e ON CAST(d.ManagerID AS NVARCHAR(10)) = e.EmployeeID
        ...
        raise HTTPException(
            status_code=500,
            detail={"Status": False, "Error": str(e), "Message": "Failed to load departments. Please try again later."}
        )
...
async def update_department(
    ...
    try:
        ...
        if not department_dict:
            raise HTTPException(
                status_code=400,
                detail={"Status": False, "Message": "No data provided for update"}
            )
        ...
        if not update_parts:
            raise HTTPException(
                status_code=400,
                detail={"Status": False, "Message": "No valid fields provided for update"}
            )
