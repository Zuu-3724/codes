    ...
    try:
        ...
        async def fetch_data():
            ...
            if hasattr(request.state, 'self_only') and request.state.self_only:
                ...
                logger.info(
                    f"Filtering salary data to only show employee ID: {employee_id}")
        ...
        raise HTTPException(
            ...
            detail={"Status": False, "Error": str(
                e), "Message": "Failed to retrieve salary information"}
        )
...
async def get_salary_history(employee_id: str, request: Request):
    ...
    try:
        ...
        logger.error(
            f"Error getting salary history for employee {employee_id}: {str(e)}")
        raise HTTPException(
            status_code=500,
            detail={"Status": False, "Error": str(
                e), "Message": "Failed to retrieve salary history"}
        )
...
async def get_attendance(request: Request):
    ...
    try:
        ...
        if hasattr(request.state, 'self_only') and request.state.self_only:
            employee_id = request.state.id
            logger.info(
                f"Filtering attendance data to only show employee ID: {employee_id}")
        ...
        raise HTTPException(
            status_code=500,
            detail={"Status": False, "Error": str(
                e), "Message": "Failed to retrieve attendance data"}
        )
...
async def update_salary(employee_id: str, data: Dict[str, Any], request: Request):
    ...
    try:
        ...
        logger.error(
            f"Error updating salary for employee {employee_id}: {str(e)}")
        raise HTTPException(
            status_code=500,
            detail={"Status": False, "Error": str(
                e), "Message": "Failed to update salary"}
        )
...
async def get_leave_statistics(employee_id: str):
    ...
    try:
        ...
        logger.error(
            f"Error getting leave statistics for employee {employee_id}: {str(e)}")
        raise HTTPException(
            status_code=500,
            detail={"Status": False, "Error": str(
                e), "Message": "Failed to retrieve leave statistics"}
        )
