...
def check_mysql_connection():
    ...
    try:
        ...
        if cursor.fetchone():
            ...
            if count > 0:
                cursor.execute(
                    "SELECT UserID, Username, Role FROM user LIMIT 5")
...
def test_login():
    ...
    try:
        ...
        logger.info(
            f"Testing login API at {api_url} with credentials: {credentials['username']}")
...
def test_payroll_api(token):
    ...
    try:
        ...
        logger.info(
            f"Payroll API response status code: {response.status_code}")
        if response.status_code == 200:
            data = response.json()
            ...
            logger.error(
                f"Payroll API request failed with status {response.status_code}")
            logger.error(f"Response: {response.text}")
            return False
    except Exception as e:
...
def test_health_api():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            logger.error(
                f"Health API request failed with status {response.status_code}")
            logger.error(f"Response: {response.text}")
            return False
    except Exception as e:
...
def test_payroll_test_api(token):
    ...
    try:
        ...
        logger.info(
            f"Payroll test API response status code: {response.status_code}")
        if response.status_code == 200:
            data = response.json()
            ...
            logger.error(
                f"Payroll test API request failed with status {response.status_code}")
            logger.error(f"Response: {response.text}")
            return False
    except Exception as e:
