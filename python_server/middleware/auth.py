...
async def verify_token(request: Request, credentials: HTTPAuthorizationCredentials = Depends(security)):
    ...
    try:
        ...
        raise HTTPException(status_code=401, detail={
                            "Status": False, "Message": "Authentication error"})
    ...
    async def role_checker(request: Request):
        ...
        if user_role not in roles:
            raise HTTPException(
                status_code=403,
                detail={
                    "Status": False, "Message": f"Access denied. Required roles: {', '.join(roles)}"}
            )
        return True
    return role_checker
        ...
        async def verify_payroll_access(request: Request):
            ...
            if request.state.role not in [Roles.ADMIN, Roles.PAYROLL_MANAGER]:
                raise HTTPException(
                    status_code=403,
                    detail={
                        "Status": False, "Message": "Only Payroll Manager and Admin can modify payroll data"}
                )
            return True
        ...
        async def verify_hr_access(request: Request):
            ...
            if request.state.role not in [Roles.ADMIN, Roles.HR_MANAGER]:
                raise HTTPException(
                    status_code=403,
                    detail={
                        "Status": False, "Message": "Only HR Manager and Admin can modify employee data"}
                )
            return True
